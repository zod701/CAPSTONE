...
- GPS를 켜면 선택한 경로 카드의 구간 선에 현재 위치를 표시한다. 경로 좌표가 있는 구간만 대상으로 하며 정확도 100m 초과, 경로 이탈, 서로 떨어진 구간의 중첩은 표시를 보류한다. 데스크톱에는 표시하지 않는다.
- 데스크톱·모바일 모두 선택한 카드의 새로고침 버튼으로 모든 승차 지점의 공공 도착 API를 조회한다. 카드 선택·위치 갱신·도착정보 목록 열기는 호출하지 않는다. 동일 정류장은 한 번만 조회하고 15초 이내 재요청을 제한한다. 도착 시간 표시와 버스 번호 정렬, 상세 팝업은 두 환경에서 동일하게 동작한다.
- 승차 지점의 시간/도착정보를 누르면 노선별 첫 차·다음 차를 표시한다. 지하철은 방향·행선지별로 구분한다. 현재 도착정보이며 탑승 가능 예측이 아니고, 경로 총시간·정렬 계산은 바꾸지 않는다. 미매칭·부분 실패·다음 차 미제공을 구분한다.
- 실제 이동 및 모바일 브라우저 검증은 별도로 필요하다. 현재 위치 정보와 도착 응답은 브라우저 저장소에 저장하지 않는다.
